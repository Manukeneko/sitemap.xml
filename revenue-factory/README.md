...
**Phase 4: アフィリエイトAI**
- 楽天ウェブサービスAPI（実クレデンシャルでライブ動作確認済み） + Amazon PA-API 5.0（AWS SigV4署名を自前実装、実アカウントでの動作未確認）から実在の商品を検索し、テーマとの関連性・収益期待値でAIが選定・スコアリング。検索前にコンテンツ企画タイトルから短いEC検索キーワードを抽出するステップを挟む
